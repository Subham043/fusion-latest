...
<form action="{{ url()->current() }}" method="get" id="searchSubmit" class="sidebar-form">
			...
			<div class="col-md-6">
				@if (isset($displaySearch) and $displaySearch == true)
                    ...
                    <div class="input-group">
                        <input type="text" name="search" id="searchChange" class="form-control" onkeyup="getUpdateList()" value="{{ request()->get('search') }}" placeholder="{{ trans('fi.search') }}..."/>
                                            </div>
    ...
    <div class="pull-right">
		...
		<div class="input-group txt" style="">
					
                      <select class="form-control" name="item" onChange="chk()">
...
<table class="alternate">
	...
	@if(file_exists(public_path('assets/barcode/').('item-checklist-'.$item->id.'-barcode.png')))
	   ...
	   CHK-{{$item->id}}</label></td>
		...
		<td>#{{$item->number}}</td>
		<td>#{{$item->formatted_invoice_date}}</td>
		@if ($item->quote()->count())
	        <td>{{ $item->quote->formatted_event_date }}</td>
	        @else
	        <td>{{ $item->getFormattedInvoiceEventDateAttribute() }}</td>
            	@endif
		...
		<td class="noprint">
			<a href="{{route('invoices.barcodePrinterSingle', $item->id)}}" target="_blank"
	...
	<tr>
            <td><label style="text-align:center;">
	   Please save the invoice to regenerate the barcode for this item.
	   <br>
	   CHK-{{$item->id}}</label></td>
