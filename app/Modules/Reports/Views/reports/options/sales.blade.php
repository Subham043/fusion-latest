    ...
    <script type="text/javascript">
        $(function () {
            $('#btn-run-report').click(function () {
                ...
                $.post("{{ route('reports.sales.validate') }}", {
                }).done(function () {
                    clearErrors();
                    ...
                    if (output_type == 'preview') {
                        $('#preview').show();
                        $('#preview-results').attr('src', "{{ route('reports.sales.html') }}");
                    }
                    else if (output_type == 'pdf') {
                        window.location.href = "{{ route('reports.sales.pdf') }}";
                    }
		    else if (output_type == 'csv') {
                        window.location.href = "{{ route('reports.sales.csv') }}";
                    }
