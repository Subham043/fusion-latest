...
namespace FI\Modules\Inventory\Barcode;

use FI\Modules\Inventory\Barcode\Exception as BarcodeException;
use FI\Modules\Inventory\Barcode\Exception as ColorException;
 ...
 * @link        https://github.com/tecnickcom/tc-lib-barcode
 */
abstract class Type extends \FI\Modules\Inventory\Barcode\Type\Convert
{
    /**
    ...
    {
        $conv = new \FI\Modules\Inventory\Color\Pdf();
        $cobj = $conv->getColorObject($color);
        if ($cobj !== null) {
            return new \FI\Modules\Inventory\Color\Model\Rgb($cobj->toRgbArray());
        }
        return null;
    }
