@section('javascript')
    ...
    <script>
        $(document).ready(function () {
			...
			@if(!empty($event->invoices))
			@if(app('request')->get('type')=='loadin')
                    {
                        //schedule
                        id: "{!! $event->id !!}",
                        title: "{!! $event->title !!}",
                        ...
                        textColor: "#fff",
                        @endif
                        url: "{!! str_replace("edit","item-checklist-print",$event->url) !!}",
                        willcall: "{!! $event->will_call !!}",
                        num: "#{!! $event->getNum() !!}",
                        //occurrences
                        oid: "{!! $event->oid !!}",
			start: "{!! date('Y-m-d H:i', strtotime($event->invoices->custom->column_8 )) !!}",
                        end: "{!! date('Y-m-d H:i', strtotime($event->invoices->custom->column_8 )) !!}",
                        //resources
		...
		@elseif(app('request')->get('type')=='loadout')
		    {
			//schedule
                        id: "{!! $event->id !!}end",
                        title: "{!! $event->title !!}",
                        ...
                        //occurrences
                        oid: "{!! $event->oid !!}",
			start: "{!! date('Y-m-d H:i', strtotime($event->invoices->custom->column_9 )) !!}",
                        end: "{!! date('Y-m-d H:i', strtotime($event->invoices->custom->column_9 )) !!}",
                        //resources
                        @if(!$event->resources->isEmpty())
                        resource: [
                                @foreach($event->resources as $resource)
